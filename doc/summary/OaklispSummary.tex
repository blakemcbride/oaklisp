...
\cfoot{}
\lfoot{\small rev. 09/17/26}

\begin{document}

\centerline{\Huge Oaklisp Summary}
\centerline{Blake McBride (blake@mcbridemail.com)}
...
Given only a base name, \emph{load} uses whichever of \emph{myfile.oa}
or \emph{myfile.oak} was modified more recently, so a source edited
after it was compiled is not shadowed by a stale object.
...
{\large \bfseries Saving and loading worlds at runtime:}
\smallskip
\begin{verbatim}
    (dump-world "snapshot.bin")  ; save current world, keep running
    (load-world "snapshot.bin")  ; replace running world (never returns)
\end{verbatim}

The running system can save its entire state with \emph{dump-world} and
replace itself with a saved world using \emph{load-world}, which never
returns.  The files are ordinary world images usable with \emph{--world}.
...
Oaklisp macros are procedural and unhygienic, more like Common Lisp
macros than Scheme's \emph{syntax-rules}.  \emph{define-syntax} takes
an ordinary expander function that receives a whole form and returns a
replacement form; the arguments are destructured like a lambda list and
the result is built with backquote/comma.  Because expansion is
unhygienic, a macro that introduces variables must avoid capture
itself, normally with \emph{gensym}.  Use \emph{local-syntax} for a
macro needed only while compiling the file in which it is defined.
