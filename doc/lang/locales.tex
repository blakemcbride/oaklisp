...
for \emph{file-name}, while \df{load} looks for
\emph{file-name}\df{.oa}, then for \emph{file-name}\df{.oak}, and finally for
\emph{file-name}.  When both \emph{file-name}\df{.oa} and
\emph{file-name}\df{.oak} exist, \df{load} uses whichever was modified
more recently, so a source file edited after it was last compiled is
not shadowed by a stale object; if their modification times are equal
the compiled file wins.}
